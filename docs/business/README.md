...
| [ICHRA+/QSEHRA+ build plan](../analysis/plus_tier_build_plan.md) | Planning doc | **Revision 9 — canonical; Part 11 governs** | Decisions D1–D39, open items O1–O42, phased build list, prioritised question list. **Later Parts govern earlier ones** | Supersedes `plus_tier.md`'s original open-item list; governs sequencing of #43. *(Row corrected twice on 2026-08-04: first from "Rev 5 / O1–O38", then again — the intermediate fix said "Revision 6 / Part 8 governs", copied from the build plan's own header, which was itself stale because Parts 9 and 10 had shipped without a header bump. **Verify this cell against the file's last `# Part N` heading, not against its header block.**)* |
