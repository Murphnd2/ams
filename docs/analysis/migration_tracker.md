...
⚠️ **Two environments' cells are NOT settled by that release, and are deliberately left unflipped:**

- **`beta_ssa` (work) reads ⬜ for V092–V094, but project memory records all three as applied locally
  in session 26 (S26-E), with V092/V093 reported as applying cleanly.** That is an **unreconciled
  discrepancy** between this tracker and the session record — one of the two is stale. It was not
  resolved in session 27, which ran no database query of any kind. **Do not treat either value as
  authoritative until a `schema_version` query settles it.** Flipping it on memory alone is exactly
  the failure mode the maintenance note below exists to prevent.
- **Demo PSP, BPO and Master read N/A** for all three, which is a scoping statement, not a backlog:
