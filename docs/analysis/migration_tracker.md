...
| 12 | `V012__role_cleanup_psp_branding_constants.sql` | Delete unused roles (6, 7, 10), rename BPO roles (remove Accelergent branding), seed LOGO_NAVBAR/LOGO_LOGIN/FAVICON constants | ✅ 2026-02-25 | ❌ NOT RUN | Prereq: V011 (BPO roles must exist). Replaces ad-hoc `production_sql_2026-02-25.sql`. |
| 13 | `V013__user_filter_presets.sql` | user_filter_preset table with 3 configurable slots per user, seeded with default presets for existing users | ✅ 2026-02-25 | ❌ NOT RUN | No prerequisites. DatabaseInitializer also updated to seed presets for new users. |
