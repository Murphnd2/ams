...
**Service Manager Session 2 (Feb 20 evening):** No database changes. All fixes were JSP/JS only (popover sanitize fix, SortableJS drag-drop). No new migration script needed.

## How to Use

1. Before deploying code changes to production, check this file for any pending migrations
2. Run scripts in the order listed (1 → 2 → 3 → service_manager)
