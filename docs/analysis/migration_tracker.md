...
⭐ **Production is current at V094 as of 2026-09-08** (session 27). Release **`v0.94.00`** applied
**V092, V093 and V094**, which had been pending on every environment since they were authored.
...
⚠️ **V096 joins V095 in the unapplied-everywhere column (2026-09-08, S32-A).** `summit_file_export`
is authored and registered but **applied to no environment, `beta_ssa` included** — unlike V095,
which is applied to `beta_ssa`. Production remains at **V094**. ⚠️ **The weekly refresh from
production drops both V095 and V096**, returning the local schema to V094; re-apply them, in order,
after any refresh. Deployment detail and the ship-together rule are in `docs/deployment_backlog.md`
[D-95](../deployment_backlog.md).

⚠️ **V095 is applied to `beta_ssa` only and is NOT applied to production.** It was verified against
the live schema immediately after applying: table created with all ten columns, the named unique
constraint `uq_summit_plan_template_map_psp_service (psp_id, service_item_id)` present and
deliberately not the primary key, both foreign keys present, **zero rows**, and `V095` registered in
`schema_version` and reflected in the `schema_info` view. ⚠️ **The weekly refresh of `beta_ssa` from
production will drop the table and every row in it** — production is at V094, so the refresh returns
the local schema to V094. Re-apply `V095__summit_plan_template_map.sql` after any refresh, and expect
any plan template mappings entered through the T202 admin screen to be gone with it.
