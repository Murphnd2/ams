...
| 1 Employer | Config: a PSP-scoped ServiceItem → administration-type mapping (CDH, COBRA, Direct Bill, Retiree), with an admin screen. None exists today — `ServiceItem.code` is null on Setup items (s47b Q10). Copy the V095 `summit_plan_template_map` + `SummitPlanTemplateAdmin` pattern. Can be built before SDX-27: it is a gated admin page nothing reads until the emitter ships. Entering its rows is a data-entry note for Kevin. | Claude Code | ✅ Built s47i; walk 2026-09-11, KEVIN-UI, local, 6/6 passed: the cross-links both ways, add, duplicate refusal, edit, and remove with confirm. | s47b Q10/D; D46 |
...
| 1 Employer | Guidance (s47i, corrected 2026-09-11): refuse when no flag is true after the union — not merely when no elected item has a row, because an all-false row would otherwise pass. | Claude Code | ⬜ | s47i |
| 1 Employer | Model: a line-of-service ServiceItem used for quoting (e.g. FSA) maps to nothing, with no template row and no flag row. Its enhancements (e.g. HFSA, DCAP) each carry their own template and flag rows. An elected item with no row does nothing, by design. | Kevin | ✅ | Kevin 2026-09-11 |
| 1 Employer | Emitter, at the build: file 2 warns when an elected item matches no template. For line-of-service items like FSA, that warning fires on every setup. Decide whether an "intentionally no plan" marker is needed. | Claude Code | ⬜ at the emitter build | s47j |
...
| 2 Plans | Check the template mapping's service item 12 (labelled "Health FSA", template 1000). If 12 is the FSA line-of-service item, remove its row and map HFSA `123054` → 1000, keeping key segment `FSA` so the plan's `Import Plan ID` is unchanged. Otherwise a DCAP-only FSA setup would get a Health FSA plan. S45 recorded that `123054` matched no template. | Kevin | ⬜ | s47j |
...
| 3 Request | Runtime walk of build 1 | Kevin | ✅ 2026-09-11, KEVIN-UI, local. **Passed:** panel before any request; the compose page; Send (returns to the setup, status line updates, email arrives with a working link, logged on the activity); the branded upload page; the clean upload (10 rows); the issues upload (3 flagged, no values echoed); the missing-column upload (unreadable, City named); Revoke (the old link shows inactive); re-request and re-upload on a new token. **Storage verified by SQL:** earlier submissions are `SUPERSEDED` with `rows_json` NULL, and SSN, DOB, pay and department strings are absent from `rows_json`. Requester notification emails confirmed in the build 2 walk, 2026-09-11: 3 of 3 received. **Not walked separately:** a garbled token; `resolveActive` returns the same empty result for unknown and closed tokens (CODE). | S47 walk |
| **4 Census** | Build 2: review page, Load and Reject, the Clear guard (decision f) | Claude Code | ✅ Built s47f, fixed s47g; walk 2026-09-11, KEVIN-UI, local. **Passed:** unreadable upload → review (missing columns, Reject only) → Reject (logged Waiting on Them; the link stays open); the issues upload shows labels ("Last name"), is highlighted on review, and Load is disabled with its reason; clean upload review, with the roster comparison reading "First Last · …"; Load disabled when Demographics is settled, and the Replace checkbox hidden; Clear refused by the D45 f guard, with the roster unchanged (6); the inactive page's title and footer have no stray separators. | T231; D45 e–f |
| 4 Census | A client upload always logs a Received Email entry. The status follows the outcome: clean → Waiting on Us; issues or unreadable → Waiting on Them (Kevin, 2026-09-11 walk). | Claude Code | ✅ | Kevin 2026-09-11 / S47 walk |
...
| 4 Census | Runtime walk of a successful Load: into an empty roster, then as a confirmed replacement, on a setup whose Demographics is not pushed or done. It isn't walkable on ZZTESTCompany 9102 by design (Demographics is DONE MANUAL, with a 6-participant roster). | Kevin | ⬜ | S47 build 2 walk |
...
| **Open questions** | Answered s47e: the upload page and request-email signature carry the agency (SWBD) via `OriginatingAgencyResolver`; `/apply/*` resolves the agency by a different rule (prospect.agent, PSP-filtered), recorded, not changed. The email wrapper footer is PSP-wide. | Claude Code (one read) | ✅ answered s47e | S47 walk item 4 |
| Open questions | Should client-facing links (census request and proposal) use the agency's `landing_host` (e.g. premiumpath.net) instead of the admin's host? Today both use the admin's host (`SendProposal.java:60`, `CensusRequestServlet` base URL). Decide before the Forrest demo. | Kevin (decision) | ⬜ | s47e Q14(d) |
