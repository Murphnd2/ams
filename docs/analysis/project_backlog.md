...
| 42 | HealthSherpa ICHRA integration — evaluation | MED | 💡 Backlog (eval) | Read-only eval before any build: (1) grab the free QuoteConnect API key, sanity-test plans/rates for **Hopkins County, TX** (Sandoval group) + confirm off-exchange carrier coverage; (2) onboarding-rep conversation — **AOR attribution**, **BAA/PHI**, carrier network, production access. HealthSherpa = CMS-approved EDE / ICHRA connectivity layer (supplier, not competitor); would let AMS be a quote-to-admin ICHRA platform. **Policy Status API is a candidate unblock for #38** (kills monthly attestations). No AMS integration commitment until eval clears; then a Phase-A investigation. See `docs/business/healthsherpa.md`. Logged 2026-07-28. **Update 2026-07-28:** quoting eval DONE — HealthSherpa One (`api.one.healthsherpa.com`, `POST /v1/quotes`) returns Forrest's carriers (Christus/Ambetter/BCBS) at Hopkins TX and matched the zizzl baseline (BCBS Blue Advantage Silver HMO 306, off-exchange) at $582.78 exactly. On-ex + off-ex quoting both open on the free tier. Remaining = rep conversation only: off-ex enrollment approval (`api_enrollable` gate) + AOR/BAA (agent-of-record derived server-side from linked HealthSherpa agent). |
