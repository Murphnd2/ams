...
| **4o** | ⭐ Run a full illustration, then **change the ZIP** | the **whole results panel disappears** along with the chooser and the county selection | ⚠️ **the old rate table is still on screen under the new chooser.** That is **W1** — observed as a Hopkins table sitting below a Collin/Denton chooser, with only the small footer line naming the county it belonged to |
| **4p** | Pick a county from the chooser | the **ZIP stays in the field** (**W5**) | the field blanks — the page forgets what the agent just typed |
| **4q** | Type a ZIP resolving to **one** county that has no rates | *"We don't have rates for X, TX yet"* as a panel, and **no confirmation line under the ZIP field** (**W4**) | a county name appears under the field and knocks the input row out of alignment |
| **4r** | Run any illustration with **no ZIP typed**, then read the `illustration_log` row | **`zip_code` is NULL** (**W2**) | it holds the county's representative ZIP — which reads as something the agent typed and was not |
...
| **10a′** | ⭐ **Before touching anything, look at the results page for five seconds.** Ask: is there a control here? | the contribution slider is **noticed without being pointed out** — tinted card, filled track, a handle that reads as draggable, and ticks on the track | ⚠️ **you look past it to the numbers.** That is **W10**, and it is a demo failure, not a cosmetic one: the agent who watched this get built said *"I didn't get the impression that the page was anything other than a static result."* A control nobody notices is one nobody drags, and a demo where nobody drags it is a demo of a calculator |
| **10a″** | Look at the tick marks on the track | one per distinct flip contribution, labelled *"age N"*, **all identical in colour and size**, with the legend *"Marks on the track show where each age band's affordability verdict changes"* | any tick, colour band or label implies one contribution is **recommended, optimal or better** — that is steering, and it is the boundary this feature sits closest to |
| 10b | Drag the **Employer Monthly Contribution** slider | net cost, group total, outlay and each verdict all move live; no page reload; the "Use This in a Proposal" link follows the slider; the track fill follows the handle | nothing moves (script error — check the console), or the verdict column stays frozen while net cost moves |
| **10c** | Hover the greyed **"Use This in a Proposal"** control | `not-allowed` cursor, **no hover response at all** | it lights up on hover — the universal "this works" signal on a control that does not (**W11**) |
