...
| **G2** | **Affordability's two constants are seeded nowhere.** `ICHRA_AFFORDABILITY_PCT_<year>` and `FPL_ANNUAL_<year>` appear in exactly one file — `IllustrationServlet.java:373,381`, the reader. No migration, no `DatabaseInitializer`, no seed script. Absent → "Affordability is not configured for plan year N" and no table | **5** | operator config only — a `constant` row per plan year | ❌ config, not code (no migration this run) | XS for Kevin, **blocking** |
...
| **G9** | **Step 5 says "moves the contribution slider."** There is no slider — contribution is a number input and each change is a full form re-submit (`illustration25.jsp:152-156`) | **5** | `illustration25.jsp` | ✅ but out of bar | S–M |
...
| 10 | Re-enter county/ages/contribution, **Illustrate** | the **Affordability Threshold** block with flip contributions | **"Affordability is not configured for plan year 2026"** → **G2: the two `constant` rows are missing. This is the single most likely thing to break step 5, and it is config Kevin must add.** |
