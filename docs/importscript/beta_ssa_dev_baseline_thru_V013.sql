...
CREATE TABLE `task` (
  ...
  `source_owner` bigint DEFAULT NULL,
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `ac_base_11` AS select count(`a`.`todo_id`) AS `counts`,`a`.`owner_id` AS `owner_id`,`a`.`source_owner` AS `source_owner`,`a`.`activity_id` AS `activity_id` from `ac_base_10` `a` group by `a`.`activity_id`,`a`.`owner_id`,`a`.`source_owner` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `acx_open_activities` AS select `a`.`id` AS `id`,`a`.`DTYPE` AS `DTYPE`,`a`.`full_name` AS `full_name`,`a`.`first_name` AS `first_name`,`a`.`last_name` AS `last_name`,`a`.`employee_id` AS `employee_id`,`a`.`due_date` AS `due_date`,`a`.`is_complete` AS `is_complete`,`a`.`assigned_to_id` AS `assigned_to_id`,`a`.`employer_id` AS `employer_id`,`a`.`last_status_id` AS `last_status_id`,`a`.`last_outbound` AS `last_outbound`,`a`.`checklist_id` AS `checklist_id`,`n`.`owner_id` AS `task_owner_id`,`n`.`source_owner` AS `source_owner_id`,(case when (`n`.`owner_id` is null) then concat(`a`.`id`,'-',`a`.`assigned_to_id`,'-N') when (`n`.`source_owner` is null) then concat(`a`.`id`,'-',`a`.`assigned_to_id`,'-',`n`.`owner_id`,'-N') else concat(`a`.`id`,'-',`a`.`assigned_to_id`,'-',`n`.`owner_id`,'-',`n`.`source_owner`) end) AS `UID` from (`ac_base_09` `a` left join `ac_base_11` `n` on((`a`.`id` = `n`.`activity_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `ck_base_04` AS select `c`.`todo_count` AS `todo_count`,`c`.`checklist_id` AS `checklist_id`,`c`.`full_name` AS `full_name`,`c`.`due_date` AS `due_date`,`c`.`assigned_to_id` AS `assigned_to_id`,`c`.`todo_id` AS `todo_id`,`c`.`task_id` AS `task_id`,`c`.`name` AS `name`,`c`.`has_owner` AS `has_owner`,`c`.`is_sourced` AS `is_sourced`,`c`.`owner_id` AS `owner_id`,`c`.`allow_non_owner` AS `allow_non_owner`,`c`.`allow_early` AS `allow_early`,`c`.`allow_future` AS `allow_future`,`c`.`sort_order` AS `sort_order`,`c`.`min_sort_top` AS `min_sort_top`,`c`.`at_top` AS `at_top`,`c`.`min_sort_block` AS `min_sort_block`,`c`.`is_blocked` AS `is_blocked`,`c`.`checklist_assigned_to_id` AS `checklist_assigned_to_id`,`c`.`checklist_owner` AS `checklist_owner`,`c`.`has_goto` AS `has_goto`,`c`.`goto_link_id` AS `goto_link_id`,`c`.`has_info` AS `has_info`,`c`.`info_link_id` AS `info_link_id`,`c`.`auto_id` AS `auto_id`,`c`.`has_automation` AS `has_automation`,`c`.`source_owner` AS `source_owner`,`c`.`not_me` AS `not_me` from `ck_base_03` `c` where ((`c`.`not_me` = true) and (`c`.`is_blocked` = false)) order by `c`.`due_date`,`c`.`checklist_id`,`c`.`sort_order`,`c`.`todo_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `ckx_open_checklists` AS select `c`.`todo_count` AS `todo_count`,`c`.`checklist_id` AS `checklist_id`,`c`.`full_name` AS `full_name`,`c`.`due_date` AS `due_date`,`c`.`assigned_to_id` AS `assigned_to_id`,`c`.`days_in_advance` AS `days_in_advance`,`a`.`owner_id` AS `owner_id`,`a`.`source_owner` AS `source_owner`,`a`.`has_owner` AS `has_owner`,`a`.`is_sourced` AS `is_sourced`,(case when (`a`.`owner_id` is null) then concat(`c`.`checklist_id`,'-',`c`.`assigned_to_id`,'-N') when (`a`.`source_owner` is null) then concat(`c`.`checklist_id`,'-',`c`.`assigned_to_id`,'-',`a`.`owner_id`,'-N') else concat(`c`.`checklist_id`,'-',`c`.`assigned_to_id`,'-',`a`.`owner_id`,'-',`a`.`source_owner`) end) AS `UID`,(case when (`c`.`days_in_advance` is null) then `c`.`due_date` else (`c`.`due_date` - interval `c`.`days_in_advance` day) end) AS `show_date`,`c`.`todo_count` AS `todo_count_f` from (`ck_base_05` `c` left join `ck_base_04` `a` on((`c`.`checklist_id` = `a`.`checklist_id`))) order by `c`.`due_date`,`c`.`full_name`,`a`.`sort_order` desc */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `td_base_03` AS select `t`.`todo_id` AS `todo_id`,`t`.`date_completed` AS `date_completed`,`t`.`is_complete` AS `is_complete`,`t`.`sort_order` AS `sort_order`,`t`.`checklist_id` AS `checklist_id`,`t`.`completed_by_id` AS `completed_by_id`,`t`.`task_id` AS `task_id`,`t`.`allow_future` AS `allow_future`,`t`.`min_sort_top` AS `min_sort_top`,`t`.`min_sort_block` AS `min_sort_block`,`a`.`owner_id` AS `owner_id`,`a`.`is_sourced` AS `is_sourced`,`a`.`source_owner` AS `source_owner`,`a`.`allow_early` AS `allow_early`,`a`.`has_owner` AS `has_owner`,`a`.`auto_id` AS `auto_id`,`a`.`has_goto` AS `has_goto`,`a`.`goto_link_id` AS `goto_link_id`,`a`.`has_info` AS `has_info`,`a`.`info_link_id` AS `info_link_id`,`a`.`has_automation` AS `has_automation`,`a`.`allow_non_owner` AS `allow_non_owner`,`a`.`DESCRIPTION` AS `name` from (`td_base_02` `t` join `task` `a` on((`t`.`task_id` = `a`.`task_id`))) */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
/*!50001 CREATE ALGORITHM=UNDEFINED */
/*!50013 DEFINER=`root`@`localhost` SQL SECURITY DEFINER */
/*!50001 VIEW `tdx_open_list` AS select `t`.`todo_id` AS `todo_id`,`t`.`task_id` AS `task_id`,`t`.`name` AS `name`,`t`.`has_owner` AS `has_owner`,`t`.`is_sourced` AS `is_sourced`,`t`.`owner_id` AS `owner_id`,`t`.`allow_non_owner` AS `allow_non_owner`,`t`.`allow_early` AS `allow_early`,`t`.`allow_future` AS `allow_future`,`t`.`sort_order` AS `sort_order`,`t`.`min_sort_top` AS `min_sort_top`,`t`.`at_top` AS `at_top`,`t`.`min_sort_block` AS `min_sort_block`,`t`.`is_blocked` AS `is_blocked`,`t`.`checklist_id` AS `checklist_id`,`t`.`checklist_name` AS `checklist_name`,`t`.`is_checklist_complete` AS `is_checklist_complete`,`t`.`checklist_due_date` AS `checklist_due_date`,`t`.`checklist_assigned_to_id` AS `checklist_assigned_to_id`,`t`.`DTYPE` AS `DTYPE`,`t`.`is_activity_complete` AS `is_activity_complete`,`t`.`activity_owner_id` AS `activity_owner_id`,`t`.`checklist_owner` AS `checklist_owner`,`t`.`has_goto` AS `has_goto`,`t`.`goto_link_id` AS `goto_link_id`,`t`.`has_info` AS `has_info`,`t`.`info_link_id` AS `info_link_id`,`t`.`auto_id` AS `auto_id`,`t`.`has_automation` AS `has_automation`,`t`.`source_owner` AS `source_owner`,(case when (((`t`.`has_owner` = true) or (`t`.`is_sourced` = true)) and (`t`.`owner_id` is not null) and (`t`.`owner_id` <> `t`.`checklist_assigned_to_id`)) then 1 else 0 end) AS `not_me` from `td_base_05` `t` where ((`t`.`DTYPE` = 'Person') and (`t`.`is_checklist_complete` = false)) order by `t`.`checklist_due_date`,`t`.`checklist_id`,`t`.`sort_order`,`t`.`task_id` */;
/*!50001 SET character_set_client      = @saved_cs_client */;
/*!50001 SET character_set_results     = @saved_cs_results */;
/*!50001 SET collation_connection      = @saved_col_connection */;
...
-- Dump completed on 2026-02-25 16:54:24
