...
- **Session 42 (2026-09-10, one migration V097, local only — not released):** **T229 (push to DataPath from the Summit setup panel) and T227 (delivery guards) are built and committed** (`4d8e7e3`), gated on `SUMMIT_PUSH_ENABLED=true`. **V097** adds four delivery columns to `summit_file_export`; it is **applied to local `beta_ssa` only** (Kevin, via MySQL Workbench, 09:38) — **not on production**. **Runtime-verified locally against the live Summit tenant**: push #8 `ZZ_TEST_ER_20260910111246.txt` was processed by Summit (SDX-18's first success token, `Successful|ZZTESTCompany 9102|158E140952|Employer edited successfully`), while push #5 was held as a content duplicate — both confirming the push path end to end. ⭐ **SDX-19 is corrected**: Summit's Schedule Import checkbox gates the ~15-minute `ImportFiles` poll (new: SDX-22, open) — contrary to DataPath's prior "non-functional" answer, which is why an earlier push appeared not to be picked up. **Session 43 opens on T230** (response check and Mark done) — unblocked, needs its own Phase A, touches `SummitExportServlet` and the panel. Release `v0.97.00` (WAR + V097) and D-96 (`SUMMIT_PUSH_ENABLED` on production) remain Kevin's to do. Full detail: `docs/session_closeout_2026-09-10_session42.md`.
- **Session 41 (2026-09-10, no migration):** **Production is on `v0.96.01`**, deployed by `update.sh` 2026-09-10 12:34:36, `DONE` logged, no migration attached (still V096). **The T201 guard is live** — `SummitExportServlet`'s `type=enrollment` refuses with a plain-text 400 unless the request carries `confirm=ENROLL-ALL-P{proposalId}`; production-verified against proposal 136814 (prospect 136810, 0 participants), refusal returned, no file produced. **The Setup detail screen now has a collapsible Summit setup panel** (`detailSummitSetup25.jsp`, PSP-admin only) replacing the old four-button export block and the Census Upload card; its not-built-yet placeholders map to backlog rows **T228–T233**. Full detail: `docs/session_closeout_2026-09-10_session41.md`.
...
- ⭐ **Summit export status (session 29, 2026-09-08): the import chain is PROVEN END TO END from AMS-generated files.** The line *"generation is proven, acceptance is not"* — carried since session 27 — **is retired.** Files 1 (Employer Demographic), 2 (Employer CDH Plan) and 4 (Demographics) were generated by AMS, uploaded to the **live** Summit tenant, and accepted; file 4's last run returned six rows, five `Participant Successfully Edited` and one `Created`, zero failures. ⚠️ **The counter-headline: the enrollment file has no AMS emitter.** `SummitExportServlet` supports exactly three `type` values (`employer`, `cdhplan`, `demographics`) — **AMS emits three of four**, and HRA Enrollment has only ever been imported hand-built. That emitter is the recommended next build. ⚠️ **`125 PI Elections`, not HRA Enrollment, is the correct import type for `Ins125` plans** — but that rests on Summit vendor AI (2026-09-07), never test-verified, so establish its field set by import before building on it. **Three shape facts, all import-established, all in `docs/business/summit_data_exchange.md`:** (1) `Employer TPA Custom ID` **must be alphanumeric** and is now `{prefix}E{prospectId}` (`158E140952`) — hyphen and underscore are both rejected *before field binding*; (2) **the constraint does not generalise** — `Import Plan ID` and `Participant TPA Custom ID` both accept hyphens, the latter proven through enrollment, so `{prefix}-P-{id}` is unchanged; (3) Demographics emits **twelve columns in the template's A–L order** ending in a mandatory `Branch Code` sentinel, because a trailing empty optional field breaks Summit's parse. **Two new `ssa.properties` keys, both defaulting safely:** `SUMMIT_IMPORT_TEMPLATES` (D-91, filename prefixes — unset preserves legacy names) and `SUMMIT_BRANCH_CODE` (D-92, defaults `AMS`). ⚠️ **D-92 is LOW for AMS but HIGH for Summit** — the tenant's Demographics template must map `Branch Code` last or file 4 does not import at all. ⚠️ **Summit dedupes a re-sent file on CONTENT, not filename** (T194), so a retry after a Summit-side fix is a silent no-op. Full detail: `docs/session_closeout_2026-09-08_session29.md`.
...
- **Session 42 (2026-09-10, S42-A through S42-E, one migration V097, no SQL executed by Claude
  Code):** Built **T229** — push to DataPath from the Summit setup panel, as a POST mode of
  `SummitExportServlet` (`doPost` sets a request attribute, `doGet` shares every check) gated on
  `SUMMIT_PUSH_ENABLED=true` — and **T227**'s delivery guards: a pushed-filename collision refusal
  (installation-wide, since there is one `ImportFiles`) and a per-PSP duplicate-content refusal
  with an acknowledgement override, landing **V097**'s four `summit_file_export` delivery columns
  (`4d8e7e3`). ⭐ **Runtime-verified locally against the live Summit tenant**: push #8
  `ZZ_TEST_ER_20260910111246.txt` was processed by Summit — **SDX-18's first observed success
  token**, `Successful|ZZTESTCompany 9102|158E140952|Employer edited successfully` — while push #5
  was held as a content duplicate. ⚠️ **Three prior claims were withdrawn on evidence, not
  guessed at:** (1) SDX-19's "Schedule Import does not function" (DataPath, CASE-22040) — Kevin
  found the checkbox gates the ~15-minute poll (new: SDX-22, open); (2) S42-D's diagnosis that the
  earlier `_P{id}_` filename shape never matched a template — it did match and was held, not
  rejected, the diagnosis was wrong and its false code comment was corrected before commit; (3)
  S42-D's own compliance statement that V097 was unapplied locally — Kevin had applied it via
  Workbench at 09:38, before any of the session's runtime tests ran. Not runtime-verified: the
  acknowledgement path, `PUSH_FAILED`, the pushed-filename refusal itself, and the `cdhplan`/
  `demographics` pushes (same code path, untested). **Session 43 opens on T230** — response check
  and Mark done; unblocked, needs its own Phase A. Full detail:
  `docs/session_closeout_2026-09-10_session42.md`.
- **Session 41 (2026-09-10, S41-A through S41-I, no migration, no SQL):** Built the **T201 guard**
  in front of `writeHraEnrollment` (`721864b`, runtime-verified: Setup-link-no-token refused,
  wrong-proposal-token refused, correct-token passed through to the emitter's first-ever runtime
  execution), corrected `summit_data_exchange.md`'s stale no-UI-link claim (`5a70504`), wrapped the
  Services To Implement badges (`5e40e6f`), and replaced the Setup screen's export buttons with a
  collapsible **Summit setup panel** (`c820ed5`, new include `detailSummitSetup25.jsp`) whose
  placeholders were filed as **T228–T233** (`040ec28`). Shipped to production as **`v0.96.01`**
  (2026-09-10 12:34:36, no migration), production-verified. Full detail:
  `docs/session_closeout_2026-09-10_session41.md`.
...
- **Session 29 (2026-09-08, S29-A–K, 12 sub-runs, no migration, no SQL):** ⭐ **The Summit import
  ...
  (T187 `pageEncoding` + T188 elected-service count/clip on `detailSetup25.jsp`), `26dc404`
  (config-driven export filenames, new `SummitImportTemplateResolver`, D-91), `892e08a` (employer key
  → `{prefix}E{prospectId}`, T192), `4c66f74` (Demographics A–L order + `Branch Code` sentinel, D-92,
  T193), `754f49d` (close-out + T194).
  ⭐ **The session's reusable lesson: field-level constraints on a third-party system do not
  generalise from one field to its neighbours, and the cost of testing one was a single file.** S29-G
  assumed the alphanumeric constraint would carry from `Employer TPA Custom ID` to
  `Participant TPA Custom ID` *"because they are the same family of field."* **Four imports disproved
  it** — three identifiers, three different validations. Reasoning produced the wrong answer; uploading
  a file produced the right one in minutes.
  ⭐ **Two sub-runs hard-stopped before writing code and both were right to.** S29-G's discovery gate
  caught that `Import Plan ID` is composed *from* `employerTpaCustomId` and could not have been left
  unchanged — converting a guess into a question, and the replacement prompt was **smaller**, not
  larger. S29-A stopped on T187 because the bytes were **already correct UTF-8** (`e2 80 94`) and the
  fault was a missing `pageEncoding`: ⚠️ **the obvious one-line "fix mojibake" edit would have replaced
  correct bytes with wrong ones** and left the real cause for the next non-ASCII character anyone
  typed. That exposure is filed as **T191** — no JSP in this repo declares `pageEncoding` and
  `web.xml` sets no `<jsp-config>`, so the container default (ISO-8859-1) mojibakes any non-ASCII
  character in any of them.
  ⚠️ **A doc contradiction was INTRODUCED by this session and closed within it.** S29-G2's fence
  permitted only `summit_data_exchange.md`'s ID-ownership section, so the file briefly asserted the new
  employer shape in one section and the old one in another; S29-H corrected the second passage before
  the commit shipped. Also corrected: the stale File 4 **"blocked"** claim (V094's roster resolved it;
  the file has since imported twice) and a nine-column Demographics example that disagreed with the
  twelve the template actually maps.
  **Evidence class is unusually strong** — results came from a live third-party system, not a browser
  walk. ⚠️ **Still only code-verified:** a non-default `SUMMIT_BRANCH_CODE`, the prefix-collision
  warning, the malformed-entry and duplicate-discriminator paths in `SummitImportTemplateResolver`, and
  the export path for any employer other than the one test company. **File 3 (Premium Billing) remains
  entirely unproven.**
  ⚠️ **Live-tenant test artifacts need cleanup** — employers `158140952` and `158E140952`; plans
  `158140952-PROBEA-2026`, `158140952PROBEB2026`, `158E140952-FSA-2026`, `158E140952-DCAP-2026`;
  participants `158-P-9001`, `158P9002`, `158-P-77`–`158-P-82`; and the enrollments against
  `158140952`. **`158140952` predates the `E` and is not a precedent.** ⚠️ **Summit does NOT validate
  plan name against plan type** — it accepted plans named "Health FSA 2026" on the `ICHRA+` template,
  so **a wrong template id will never be caught by an import** and D-90's mapping has no import-time
  safety net. **Four defects filed: T191, T192, T193** (Summit enforces field lengths AMS does not —
  `Mailing Address Line 1` caps at 50, 48 passes, every other field untested), **T194.** Close-out:
  `docs/session_closeout_2026-09-08_session29.md`.
