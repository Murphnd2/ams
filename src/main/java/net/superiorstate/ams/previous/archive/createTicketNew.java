...
@WebServlet(name = "createTicketNew", value = "/createTicketNew")
public class createTicketNew extends HttpServlet {
    ...
    private void createTicketAlt(HttpServletRequest request){
        ...
        viewCreatedTicket(em,t,request);

        em.close();
    }

    private void viewCreatedTicket(EntityManager em, Ticket t, HttpServletRequest request){
        ...
        try{
            ...
            activityOutList = null;
        }
        if(activityOutList!=null)
            for(ActivityOut a1:activityOutList)
                if(a1.getActivity().getId().equals(t.getId()))
                    em.refresh(a1);
        sVar.setOpenActivityList(activityOutList);
        sVar.reFilterActivityList();
        sVar.setCurrentActivity(t);
        request.getSession().setAttribute("activityOwner1",t.getAssignedTo());
