...
@WebServlet(name = "RemoveAgent", value = "/RemoveAgent")
public class RemoveAgent extends HttpServlet {
    ...
    private void removeAgent(HttpServletRequest request,HttpServletResponse response){
        ...
        long personId = Long.parseLong(request.getParameter("activeAgentList"));
        Person person = dM.getPersonById(em,personId);

        UserRole urAgent = dG.getUserRoleById(em,2);
        UserRole urPending = dG.getUserRoleById(em,6);
        ...
        agency.removeAgent(person);
        User u = dbAuth.getUserFromPerson(em,person);
        u.removeUserFromRole(urAgent);
        u.addUserToRole(urPending);
