...
@WebServlet(name = "ViewRecurringSequence", value = "/ViewRecurringSequence")
public class ViewRecurringSequence extends HttpServlet {
    @Override
    ...
    private void viewSequence(HttpServletRequest request){
        ...
        Long seqId = Long.parseLong(request.getParameter("recListSelection"));
        request.getSession().setAttribute("currentReqList", new RequiredTaskList());
        request.getSession().setAttribute("currentRecList", dbRec.getRecurringListById(em,seqId));
