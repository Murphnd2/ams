...
@WebServlet(name = "TaskBuilder", value = "/TaskBuilder")
public class TaskBuilder extends HttpServlet {
    ...
    private void updateSequence(HttpServletRequest request,EntityManager em){
        ...
        if(isNewList){
            if(radioTikRenSet.equals("0")){
                ...
                tp.setSortOrder(100);
                assert tc != null;
                tp.setDescription(tc.getShortText()+":"+textTikNew);
    ...
    private void saveTaskState(HttpServletRequest request, EntityManager em){
        ...
        if(currentList==null || currentList.size()==0)
            return;
        List<GenSeq> newList = new ArrayList<>();
        boolean rbExists;
        Task t;
        for(int x = 0; x < currentList.size(); x++){
            ...
            if(gs.getTask()==null){
                ...
                String holder = "whichRadio" + x;
                rbExists = request.getParameter(holder)!=null && request.getParameter(holder).equals("1");
                System.out.println("Task Exists: "+rbExists);
                if(rbExists){
                    t = dM.getTaskById(em,Long.parseLong(Objects.requireNonNull(request.getParameter("selTask"+x))));
                } else{
                    PSP psp = dM.getPspById(em,4L);
                    em.getTransaction().begin();
                    t = new Task();
                    t.setDescription(Objects.requireNonNull(request.getParameter("taskDesc"+x)));
                    t.setReUsable(request.getParameter("saveTheTask"+x)!=null);
                    t.setAllowFuture(true);
                    t.setAllowEarly(true);
                    t.setHasInfo(false);
                    t.setHasOwner(false);
        ...
        request.getSession().setAttribute("listBuilder",newList);
        System.out.println("List Count: " + newList.size());
    }
    ...
    private void moveTaskUp(HttpServletRequest request, EntityManager em, int start){
        ...
        startList.get(start).setSequenceNumber(start-1);
        endList.add(startList.get(start));
        startList.get(start-1).setSequenceNumber(start);
    ...
    private void addTaskToSequence(HttpServletRequest request, EntityManager em){
        ...
        gs.setSequenceNumber(taskIndex+1);
        gs.setDescription("");
        endList.add(gs);
