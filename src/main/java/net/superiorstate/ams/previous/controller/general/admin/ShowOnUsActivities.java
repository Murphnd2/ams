...
@WebServlet(name = "ShowOnUsActivities", value = "/ShowOnUsActivities")
public class ShowOnUsActivities extends HttpServlet {
    @Override
    ...
    private void changeView(HttpServletRequest request){
        request.getSession().setAttribute("aFilter","OnUs");
        int onUs = (Integer) request.getSession().getAttribute("onUs");
        if(onUs==1){
            request.getSession().setAttribute("onUs",0);
        } else {
            request.getSession().setAttribute("onUs",1);
