...
@WebServlet(name = "SendHSAWelcomeSetup", value = "/SendHSAWelcomeSetup")
public class SendHSAWelcomeSetup extends HttpServlet {
    @Override
    ...
    private void sendQuickAction(HttpServletRequest request) throws MessagingException {
        ...
        String eSubject = "Welcome - (H)ealth (S)avings (A)ccounts";
        ActivityStatus activityStatus = dM.getActivityStatusById(em,1);
        System.out.println("Status = " + activityStatus.getDescription());

        String eMessage = Q.WELCOME_NEW_HSA + StdAuto.userSignature(p);
        Email email = StdAuto.createEmail(request,em,a,eSubject,eMessage,activityStatus,p);
        Email emailToSend = dM.getEmailById(em,email.getId());
        dbEmail.sendEmail(emailToSend,em);
        StdAuto.closeThisTask(em,a,4351L,p);
        em.close();
    }
}
