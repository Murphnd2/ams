...
@WebServlet(name = "SendEligibilityTestInitial", value = "/SendEligibilityTestInitial")
public class SendEligibilityTestInitial extends HttpServlet {
    @Override
    ...
    private void sendQuickAction(HttpServletRequest request) throws MessagingException {
        ...
        Activity a = StdAuto.getCurrentActivity(request);
        Person p = StdAuto.getCurrentPerson(request);

        String eSubject = "125 Plan Non-Discrimination Testing";
        String eMessage = Q.TEST_ELIGIBILITY_SETUP + Q.insert125Test(a) + StdAuto.userSignature(p);
