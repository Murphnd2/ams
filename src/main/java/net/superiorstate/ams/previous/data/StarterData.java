...
public abstract class StarterData {
    ...
    private static PlanType createSummitPlanType(EntityManager em, int typeId, String code, String name, int groupId, int tpId){
        ...
        BillingGroup bg = dM.getBillingGroupById(em,groupId);
   ...
   /* private static void fillStandardCopay(EntityManager em){
    ...
    public static void createTicketSubCategory(EntityManager em, Long catId, Long subCatId, String name){
        if(dM.getTicketCategoryById(em,catId)==null)
            return;
        em.getTransaction().begin();
        TicketSubCategory tsc = new TicketSubCategory();
        tsc.setId(subCatId);
        tsc.setTicketCategory(dM.getTicketCategoryById(em,catId));
    ...
    private static void assignModulesToLos(EntityManager em){
        ServiceModule smDebitCards = dG.getServModByShortText(em,"Cards");
        ServiceModule smPayment = dG.getServModByShortText(em,"Payment");
        ServiceModule smPOP = dG.getServModByShortText(em,"POP");
        ServiceModule smFSA = dG.getServModByShortText(em,"FSA");
        ServiceModule smHRA = dG.getServModByShortText(em,"HRA");
        ServiceModule smHSA = dG.getServModByShortText(em,"HSA");
        ServiceModule smCobra = dG.getServModByShortText(em,"COBRA");
        ServiceModule smTrans = dG.getServModByShortText(em,"Transit");
        LOS pop = dG.getLosByShortText(em,"POP");
        LOS fsa = dG.getLosByShortText(em,"FSA");
        LOS hra = dG.getLosByShortText(em,"HRA/MERP");
        LOS hsa = dG.getLosByShortText(em,"HSA");
        LOS cobra = dG.getLosByShortText(em,"COBRA");
        LOS transit = dG.getLosByShortText(em,"TRANSIT");
    ...
    private static void createRate(EntityManager em, Agency agency, PSP psp){
        ...
        Agency superiorState = dG.getAgencyFull(em, agency.getId());
    ...
    public static void developRateTable(EntityManager em, Rate rate){
        ServiceModule pop = dG.getServModByShortText(em,"POP");
        ServiceModule fsa = dG.getServModByShortText(em,"FSA");
        ServiceModule hra = dG.getServModByShortText(em,"HRA");
        ServiceModule hsa = dG.getServModByShortText(em,"HSA");
        PriceItem iSetup = dG.getPriceItemBySortOrder(em,100);
        PriceItem iAnnual = dG.getPriceItemBySortOrder(em,200);
        PriceItem iBase = dG.getPriceItemBySortOrder(em,300);
        PriceItem iOnline = dG.getPriceItemBySortOrder(em,280);
        PriceItem iPaper = dG.getPriceItemBySortOrder(em,290);
        //pop plans
   ...
   private static void assignRoles(EntityManager em, User user) {
        UserRole agentRole = dbAuth.getUserRoleById(em, 2);
        UserRole pspRole = dbAuth.getUserRoleById(em, 1);
        UserRole pspAdminRole = dbAuth.getUserRoleById(em, 5);
    ...
    private static void fillDaysOfTheWeek(EntityManager em){
        ddC.createDoW(em,17,"Sunday",7);
        ddC.createDoW(em,11,"Monday",1);
        ddC.createDoW(em,12,"Tuesday",2);
        ddC.createDoW(em,13,"Wednesday",3);
        ddC.createDoW(em,14,"Thursday",4);
        ddC.createDoW(em,15,"Friday",5);
        ddC.createDoW(em,16,"Saturday",6);
    }

    private static void fillLinkTypes(EntityManager em){
        ddC.createLinkType(em,1,"File Upload");
        ddC.createLinkType(em,2,"Hyperlink");
    }
