...
public abstract class eV {
    ...
    public static Person getBestPersonFromString(EntityManager em, String textBox){
        ...
        if(getEmployee(em,textBox)!=null) {
            ...
            return getPersonByEe(em, Objects.requireNonNull(getEmployee(em, textBox)));
        } else
            System.out.println("EE NULL");
        return getPerson(em,textBox);
    }

    private static Person getPersonByEe(EntityManager em, Employee ee){
    ...
    public static Employee getEmployee(EntityManager em, String textBox){
        if(hasEmployeeId(em,textBox))
            return dM.getEmployeeById(em,employeeId);
        if(hasPersonId(em,textBox) && personIsEmployee(em,dM.getPersonById(em,personId)))
            return dM.getPersonById(em,personId).getEmployee();
        if(dbEmail.isValidEmail(textBox) && emailIsEmployee(em,textBox))
    ...
    private static Person getPerson(EntityManager em, String textBox){
        ...
        if(dbEmail.isValidEmail(textBox) && isPerson(em,textBox))
            return getPersonByEmail(em,textBox);
        if(dbEmail.isValidEmail(textBox))
    ...
    public static void cleanseContactList(EntityManager em){
        ...
        for(PersonV pv: personVList){
            Employee ee = dM.getEmployeeById(em,pv.getEmployee().getId());
            em.getTransaction().begin();
            Person p = dM.getPersonById(em,pv.getId());
            p.setLastName(ee.getLastName().trim().toUpperCase());
            p.setFirstName(ee.getFirstName().trim().toUpperCase());
