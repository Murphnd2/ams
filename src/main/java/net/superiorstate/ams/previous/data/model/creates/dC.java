package net.superiorstate.ams.previous.data.model.creates;
...
public abstract class dC {
    ...
    public static RequiredTaskList createReqList(EntityManager em, String name, PSP psp, TemplatePurpose tp){
        if(dbReq.listExistsForPurpose(em,tp))
            return dbReq.getRtlForPurpose(em,tp);
        em.getTransaction().begin();
    ...
    public static WebLink createWebLink(EntityManager em, String name, String path, int linkTypeId){
        LinkType linkType = dM.getLinkTypeById(em,linkTypeId);
