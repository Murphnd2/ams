...
public abstract class dbEmail {
    ...
    public static void sendEmail(String fromWho,
        ...
        try {
            String dbg = dbA.getConstantValue(em, "SMTP_DEBUG");
        ...
        String smtpFrom = safe(dbA.getConstantValue(em, "SMTP_FROM"));
        if (smtpFrom.isBlank()) {
            ...
            // fallback: SMTP_USER is often an email at SMTP2GO
                ...
                smtpFrom = safe(dbA.getConstantValue(em, "SMTP_USER"));
        }
    ...
    private static Session getSession(EntityManager em) throws MessagingException {
        String server   = safe(dbA.getConstantValue(em, "SMTP_SERVER"));   // e.g., mail.smtp2go.com
        String port     = safe(dbA.getConstantValue(em, "SMTP_PORT"));     // e.g., 587 or 2525
        String user     = safe(dbA.getConstantValue(em, "SMTP_USER"));
        String password = safe(dbA.getConstantValue(em, "SMTP_PASSWORD"));
    ...
    public static Person getPersonByEmail(EntityManager em, String email, PSP psp) {
        ...
        if (employee != null) {
            ...
            if (p == null) dbAuth.createPersonFromEmployee(em, employee, psp);
