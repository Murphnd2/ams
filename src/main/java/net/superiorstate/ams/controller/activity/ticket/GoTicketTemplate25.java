...
@WebServlet(name = "GoTicketTemplate25", value = "/GoTicketTemplate25")
public class GoTicketTemplate25 extends HttpServlet {
    @Override
    ...
    private void doStuff(HttpServletRequest request,HttpServletResponse response){
        ...
        List<ReqTaskListTix> reqTaskListTix = new ArrayList<>();
        for(RequiredTaskList rtl: getRequiredTaskLists(em,3)){
            ReqTaskListTix rtlt = new ReqTaskListTix(em,rtl.getId());
            reqTaskListTix.add(rtlt);
        }
        reqTaskListTix.sort(Comparator.comparing(ReqTaskListTix::getDescription));

        request.getSession().setAttribute("rtlTicketList",reqTaskListTix);
        em.close();
    }
