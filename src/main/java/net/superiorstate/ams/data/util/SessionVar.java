...
import net.superiorstate.ams.model.activity.ActivityOut;
import net.superiorstate.ams.model.activity.ActivityShell;
import net.superiorstate.ams.model.activity.checklist.CheckList;
import net.superiorstate.ams.model.activity.checklist.CheckListOut;
import net.superiorstate.ams.model.activity.checklist.CheckListShell;
...
public class SessionVar {
    ...
    public void refreshUserIsIn(EntityManager em){
        setUserIsIn(TimeTrackingDAO.getMyLastPunch(em,getCurrentUser()).isIn());
    }
    public void refreshStaffList(EntityManager em){
        setStaffList(AuthDAO.getPspStaff(em,getCurrentUser().getPerson().getPsp()));
    }
    public void refreshMyTimeHistory(EntityManager em){
        setMyTimeHistory(TimeTrackingDAO.getTodaysTimeHistory(em,getCurrentPerson()));
    ...
    public void refreshOpenChecklists(EntityManager em){
        List<CheckListShell> theList = ActivityListDAO.getMyOpenItems(em,getCurrentPerson());
    ...
    public void refreshContactListFromCurrentActivity(EntityManager em){
        ...
        try{
            ...
            a1 = EntityLookup.getActivityById(em,a.getId());
            em.refresh(a1);
        } catch (Exception ignored) {}
