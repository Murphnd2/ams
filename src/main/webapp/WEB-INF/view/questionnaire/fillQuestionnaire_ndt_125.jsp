...
<head>
    ...
    <style>
        ...
        .help-icon { color: #6c757d; cursor: help; margin-left: 4px; font-size: 0.85rem; }
        .help-icon:hover { color: var(--psp-primary); }
        .section-intro { background: #f0f7fc; border-left: 3px solid var(--psp-primary); padding: 0.75rem 1rem; margin-bottom: 1rem; border-radius: 0 4px 4px 0; font-size: 0.9rem; color: #334155; }
        .section-intro .common-mistakes { margin-top: 0.5rem; padding-top: 0.5rem; border-top: 1px solid #d1e3f0; font-size: 0.82rem; color: #64748b; }
        .section-intro .common-mistakes strong { color: #b45309; font-size: 0.82rem; }
                ...
                <div class="field-group" data-field-key="${field.fieldKey}" id="fg_${field.fieldKey}">
                    ...
                    <c:if test="${field.fieldType != 'BOOLEAN'}">
                        <label for="${field.fieldKey}">
                            ...
                            <c:if test="${field.helpText != null}"><i class="bi bi-info-circle help-icon" data-bs-toggle="tooltip" data-bs-placement="right" title="${fn:escapeXml(field.helpText)}"></i></c:if>
                        </label>
                    </c:if>
                    ...
                    <c:if test="${field.helpText != null}">
                        <%-- Help text is hidden; shown as tooltip on hover icon next to label --%>
                    </c:if>
...
<script>
    ...
    // =========================================================================
    // Section descriptions (summary | common mistakes) -- custom for NDT 125
    // =========================================================================
    var sectionDescriptions = {
        'Section A - Entity & Plan Context': {
            summary: 'Tell us who the employer is and what plan year we are testing.',
            mistakes: 'Use the full legal entity name, not a DBA. The plan year end date is the last day of the plan year, not today.'
        },
        'Section B - Controlled Group / Affiliated Service Group': {
            summary: 'We need to know if your company is related to other companies. If so, employees of ALL related companies count for testing -- even if they cannot join the plan.',
            mistakes: 'Forgetting about companies owned by the same person or family members. Not counting employees at sister companies or subsidiaries.'
        },
        'Section C - Workforce Overview': {
            summary: 'How many employees does the company have, and are any of them union members?',
            mistakes: 'Including independent contractors or leased workers in the employee count. Forgetting that union employees may still need to be counted even if excluded from the plan.'
        },
        'Section D - Benefits Offered': {
            summary: 'Which pre-tax benefits does the plan offer? Your answer determines which tests apply and which sections you will see next.',
            mistakes: 'Confusing a Health FSA with an HRA. Forgetting that HSA contributions through payroll are a cafeteria plan benefit.'
        },
        'Section F - Simple Cafeteria Plan Safe Harbor': {
            summary: 'Small employers (100 or fewer) may qualify for a safe harbor that skips all testing. If you qualify, we are done after this page.',
            mistakes: 'Assuming you have a Simple Cafeteria Plan without checking the plan document. The employer must make specific contributions and meet eligibility rules.'
        },
        'Section E - Plan Eligibility Conditions': {
            summary: 'What are the plan\'s eligibility rules? Waiting periods, age requirements, and hours thresholds determine who can be excluded from testing.',
            mistakes: 'Confusing the plan\'s eligibility rules with the employer\'s general hiring policies. The eligibility rules here are what the plan document says, not HR practice.'
        },
        'Section G - Excludable Employees (Section 125)': {
            summary: 'Count employees who can be excluded from the Section 125 testing population based on the eligibility rules you described.',
            mistakes: 'Double-counting employees in multiple categories. Entering zero for a category when you said "Yes" to that eligibility rule earlier.'
        },
        'Section J - HCI Classification (Section 125)': {
            summary: 'Identify "Highly Compensated Individuals" -- owners, officers, and high earners. These are the people the tests check for favoritism toward.',
            mistakes: 'Forgetting family attribution rules -- a spouse\'s ownership counts as the employee\'s own. Not counting officers who are not owners.'
        },
        'Section N - Section 125 Eligibility Test': {
            summary: 'Does the plan offer the same eligibility to everyone, or are some groups treated differently?',
            mistakes: 'Saying "Yes" to same terms when salaried and hourly employees have different eligibility requirements.'
        },
        'Section O - Section 125 Contributions & Benefits Test': {
            summary: 'Are the same benefits and employer contributions available to everyone on equal terms? This section also collects dollar amounts for the math.',
            mistakes: 'Confusing employee salary reductions (what the employee pays pre-tax) with employer contributions (what the employer pays).'
        },
        'Section M - Key Employee Classification (Section 416(i))': {
            summary: 'Identify "Key Employees" -- a different group than HCIs, used for the concentration test. Owners, 1% owners earning over $150K, and high-paid officers.',
            mistakes: 'Confusing Key Employees (Section 416) with HCIs (Section 125). The ownership and compensation thresholds are different.'
        },
        'Section P - Section 125 Key Employee Concentration Test': {
            summary: 'No more than 25% of total plan benefits can go to Key Employees. We need their election and contribution amounts.',
            mistakes: 'Only counting salary reductions and forgetting employer contributions. Not including all Key Employee participants.'
        },
        'Section H - Excludable Employees (Section 105(h))': {
            summary: 'Count employees who can be excluded from the Section 105(h) testing population. Note: these rules are DIFFERENT from Section 125 -- different age, service, and hours thresholds.',
            mistakes: 'Using the Section 125 exclusion numbers here. Section 105(h) uses age 25 (not 21), 3 years service (not 1), and 35 hours/week (not 1,000 hours/year).'
        },
        'Section K - HCI Classification (Section 105(h))': {
            summary: 'Identify HCIs under Section 105(h) rules -- different from Section 125. Uses 10% ownership (not 5%), top 5 officers (not all), and top 25% earners.',
            mistakes: 'Using your Section 125 HCI numbers here. The thresholds are different. Re-calculate from scratch using these specific rules.'
        },
        'Section Q - Section 105(h) Eligibility Test': {
            summary: 'Three sub-tests exist -- passing any one is sufficient. We need participation counts and eligibility details for the Health FSA.',
            mistakes: 'Confusing "benefiting" (actually enrolled) with "eligible to benefit" (offered the chance). These are different numbers.'
        },
        'Section R - Section 105(h) Benefits Test': {
            summary: 'Are the same benefits, maximums, cost-sharing, and terms available equally to all participants? This checks whether the Health FSA favors HCIs.',
            mistakes: 'Forgetting about mid-year benefit changes that may have affected groups differently. Not checking if dependent coverage terms vary.'
        },
        'Section I - Excludable Employees (Section 129)': {
            summary: 'Count employees who can be excluded from the Section 129 (Dependent Care) testing population.',
            mistakes: 'Forgetting the under-$25,000 compensation exclusion, which only applies to the 55% average benefits test.'
        },
        'Section L - HCE Classification (Section 129)': {
            summary: 'Identify "Highly Compensated Employees" under Section 414(q) -- yet another definition. Used for Dependent Care FSA testing.',
            mistakes: 'Confusing HCEs (Section 414(q)) with HCIs (Section 125) or Section 105(h) HCIs. Each test uses a different definition.'
        },
        'Section S - Section 129 Eligibility Test': {
            summary: 'Is the Dependent Care FSA available to all non-excludable employees on the same terms?',
            mistakes: 'Saying "Yes" when in fact the DCFSA is only available to certain employee classes or locations.'
        },
        'Section T - Section 129 Benefits & Contributions Test': {
            summary: 'Are the same DCFSA maximum, terms, and employer contributions available equally to everyone?',
            mistakes: 'Not knowing the current statutory maximum ($5,000 for 2025, $7,500 for 2026). Forgetting to report nonelective employer contributions.'
        },
        'Section U - Section 129 More-Than-5% Owners Test': {
            summary: 'No more than 25% of total Dependent Care benefits can go to more-than-5% owners and their families.',
            mistakes: 'Forgetting to include spouse and dependent elections in the owner total. Not counting all DCAP participants in the denominator.'
        },
        'Section V - Section 129 55% Average Benefits Test': {
            summary: 'The average DCFSA benefit for non-HCEs must be at least 55% of the average for HCEs.',
            mistakes: 'Including employees who did not participate in the averages. Only participants count -- divide total elections by number of participants.'
        },
        'Section X - Notes & Attestation': {
            summary: 'Add any context that might help us understand your answers, and confirm everything is accurate.',
            mistakes: ''
        }
    };
    ...
    function getPageVisibility() {
        ...
        function sectionLetter(sectionName) {
            ...
            // Section names are like "Section A - Entity..." — extract the letter after "Section "
            var m = sectionName.match(/^Section\s+([A-Z])/);
            return m ? m[1] : '';
        }
    ...
    function showPage(index) {
        ...
        // Inject section intro (summary + common mistakes) if not already present
        var sectionBody = visiblePages[index].querySelector('.section-body');
        if (sectionBody && !sectionBody.querySelector('.section-intro')) {
            var desc = sectionDescriptions[sectionTitle];
            if (desc && desc.summary) {
                var introDiv = document.createElement('div');
                introDiv.className = 'section-intro';
                var html = desc.summary;
                if (desc.mistakes) {
                    html += '<div class="common-mistakes"><strong><i class="bi bi-exclamation-triangle me-1"></i>Common mistakes:</strong> ' + desc.mistakes + '</div>';
                }
                introDiv.innerHTML = html;
                sectionBody.insertBefore(introDiv, sectionBody.firstChild);
            }
        }
