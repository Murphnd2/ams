                  ...
                  <div class="accordion-header" id="h${list.getId()}">
                    <div class="input-group input-group-sm">
                      <c:choose>
                        ...
                        <c:otherwise>
                          <button type="submit" class="btn btn-outline-dark ${butt}" name="btnCheckList" id="btn${list.getId()}-${toDo.getId()}" value="${list.getId()}">
                            <i class="bi bi-square"></i>
                          </button>
                          <button type="submit" class="form-control ${format} btn btn-outline-dark" name="btnCheckList" id="btn2${list.getId()}" value="V-${list.getId()}">
                              ${toDo.getTask().getDescription()}
                          </button>
                          <button type="button" data-bs-toggle="collapse" data-bs-target="#collapse${list.getId()}" aria-expanded="true" aria-controls="collapse${list.getId()}" class="btn btn-warning">
                            <i class="bi bi-caret-down-square-fill"></i>
              ...
              <div class="accordion-item">
                <div class="accordion-header" id="h${list.getId()}">
                  <div class="input-group input-group-sm">
                    <c:choose>
                      <c:when test="${sessionScope.currentChecklist.getId()==list.getId()}">
                        <div type="submit" class="btn btn-dark pe-none text-warning" name="btnCheckList" id="btn${list.getId()}" value="${list.getId()}">
                          <i class="bi bi-arrow-left"></i>
                        </div>
                        <div class="form-control ${format} btn btn-dark text-warning pe-none">
                            ${list.getFullName()}
                      ...
                      <c:otherwise>
                        <button type="submit" class="btn btn-outline-dark" name="btnCheckList" id="btn${list.getId()}" value="${list.getId()}">
                          <i class="bi bi-arrow-left"></i>
                        </button>
                        <button type="submit" class="form-control ${format} btn btn-outline-dark" name="btnCheckList" id="btn2${list.getId()}" value="${list.getId()}">
                            ${list.getFullName()}
                        </button>
                        <button type="button" data-bs-toggle="collapse" data-bs-target="#collapse${list.getId()}" aria-expanded="true" aria-controls="collapse${list.getId()}" class="btn btn-warning">
                          <i class="bi bi-caret-down-square-fill"></i>
