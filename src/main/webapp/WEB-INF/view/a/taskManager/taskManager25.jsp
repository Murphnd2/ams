...
<div id="massCodeBehindNew" class="d-none">
  <c:set var="toDo" value="${sessionScope.local.getCurrentToDo()}"></c:set>
  ...
  <c:set var="isTheOwner1" value="${toDo.getTask().getOwner()!=null && toDo.getTask().getOwner().getId()==sessionScope.local.getCurrentPerson().getId()}"></c:set>
  ...
  <c:set var="isActivityOwner1" value="${sessionScope.local.getCurrentActivity().getActivity().getAssignedTo().getId()==sessionScope.local.getCurrentPerson().getId()}"></c:set>
...
<div class="container-fluid">
  ...
  <form action="UpdateTask25" method="post">
    <div class="container">
      <div class="row">
        <h3 class="col">
          MANAGE <span class="text-primary fst-italic">${sessionScope.local.getCurrentToDoOut().getDescription()} <i class="bi bi-check-square"></i></span>
        </h3>
      </div>
      ...
      <div class="accordion" id="accordionExample">
        ...
        <c:choose>
          <c:when test="${toDo.getTask().isAutomated()==true && toDo.getTask().getAutomation()==null}">
            This Task is Automated Outside of this Screen: <b>${toDo.getTask().getServletName()}</b>
            <c:if test="${sessionScope.isPspAdmin==true}">
              <a href="BlowUpAuto?taskId=${toDo.getTask().getId()}" class="btn btn-sm btn-danger">BLOW UP</a>
            </c:if>
          </c:when>
          <c:otherwise>
            <div class="accordion-item">
                ...
                <div class="accordion-body">
                  <div class="container-fluid">
                    ...
                    <div class="row row-cols-2 row-cols-md-3 row-cols-xl-4 collapse mt-1" id="XmlHelp">
                      ...
                      <div class="col mb-1">
                        <div class="card h-100 border-info">
                          <h6 class="card-header text-bg-info">&nbsp;</h6>
                    ...
                    <div class="row mt-2">
                      ...
                      <div class="col-12 col-lg-3 d-none d-lg-inline">
                        ...
                        <div class="row">
                          <div class="col" style="font-size: 0.65em">
                            <c:forEach var="refLink" items="${applicationScope.global.getInsertLinks()}">
                              <div class="row m-0 p-0 mt-1">
                                <div class="col-auto m-0 me-2 p-0">${refLink.getId()}</div>
                                <div class="col m-0 p-0" style="font-style: italic">${refLink.getPlainText()}</div>
                              </div>
                            </c:forEach>
                          </div>
                        </div>
