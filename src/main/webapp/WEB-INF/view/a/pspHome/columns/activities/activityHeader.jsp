...
<div class="p-2 pt-1 border border-dark rounded w-100 mt-2 mb-2 text-light bg-dark fw-bold fs-3 align-items-center text-center">
    <form action="ViewHome25" method="post">
        <input type="text" value="filterButton" name="formSender" hidden>
        <div class="d-none">
            <c:set var="alpha" value=""></c:set>
            <c:set var="calen" value="checked"></c:set>
            <c:if test="${sessionScope.sVar.isFilterAlphabetical()==true}">
                <c:set var="alpha" value="checked"></c:set>
                <c:set var="calen" value=""></c:set>
            </c:if>

            <c:choose>
                <c:when test="${sessionScope.sVar.getFilterWhoseActivities()==3}">
                    <c:set var="wAll" value=""></c:set>
                    <c:set var="wMePlus" value=""></c:set>
                    <c:set var="wMe" value=""></c:set>
                    <c:set var="wPlus" value="checked"></c:set>
                </c:when>
                <c:when test="${sessionScope.sVar.getFilterWhoseActivities()==0}">
                    <c:set var="wAll" value="checked"></c:set>
                    <c:set var="wMePlus" value=""></c:set>
                    <c:set var="wMe" value=""></c:set>
                    <c:set var="wPlus" value=""></c:set>
                </c:when>
                <c:when test="${sessionScope.sVar.getFilterWhoseActivities()==2}">
                    <c:set var="wAll" value=""></c:set>
                    <c:set var="wMePlus" value=""></c:set>
            ...
            <c:if test="${sessionScope.sVar.isFilterRenewal()==true}">
                <c:set var="vr" value="checked"></c:set>
            </c:if>
            <c:set var="vs" value=""></c:set>
            <c:if test="${sessionScope.sVar.isFilterSetup()==true}">
                <c:set var="vs" value="checked"></c:set>
            </c:if>
            <c:set var="vt" value=""></c:set>
            <c:if test="${sessionScope.sVar.isFilterTicket()==true}">
                <c:set var="vt" value="checked"></c:set>
            </c:if>
            <c:choose>
                <c:when test="${sessionScope.sVar.isFilterWaitingOnUs()==false && sessionScope.sVar.isFilterNeedsContact()==false}">
                    <c:set var="qou" value=""></c:set>
                    <c:set var="qnc" value=""></c:set>
                </c:when>
                <c:when test="${sessionScope.sVar.isFilterWaitingOnUs()==false}">
                    <c:set var="qou" value=""></c:set>
                    <c:set var="qnc" value="checked"></c:set>
                </c:when>
                <c:when test="${sessionScope.sVar.isFilterNeedsContact()==false}">
