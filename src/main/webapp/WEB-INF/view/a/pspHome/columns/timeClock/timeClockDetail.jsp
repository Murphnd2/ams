...
<form method="post" action="goPspHome">
  <div class="d-none" id="codeBehindClock">
    ...
    <c:if test="${sessionScope.sVar.getUserIsIn()==true}">
      <c:set var="oClass" value="btn-outline-danger"> </c:set>
      <c:set var="iClass" value="btn-success pe-none"> </c:set>
    </c:if>
    <input type="text" name="formSender" id="formSenderTimeClock" value="timeClock" hidden>
  </div>
    ...
    <div class="row">
      <div class="col">
        <div class="btn btn-outline-dark border-0 w-100 pe-none">
          <c:choose><%----%>
            <c:when test="${sessionScope.sVar.getUserIsIn()==true && sessionScope.sVar.getMyTimeHistory().size()==0}">
              IN Since Yesterday
            </c:when>
            <c:when test="${sessionScope.sVar.getUserIsIn()==true}">
              IN Since <fmt:formatDate value="${sessionScope.sVar.getMyTimeHistory().get(0).getInTime()}" pattern="hh:mm aa"></fmt:formatDate>
            </c:when>
            <c:when test="${sessionScope.sVar.getUserIsIn()==false && sessionScope.sVar.getMyTimeHistory().size()==0}">
              OUT Since Yesterday
            </c:when>
            <c:otherwise>
              OUT Since <fmt:formatDate value="${sessionScope.sVar.getMyTimeHistory().get(0).getOutTime()}" pattern="hh:mm aa"></fmt:formatDate>
            </c:otherwise>
          </c:choose>
        </div>
      </div>
...
<c:forEach var="timeStretch" items="${sessionScope.sVar.getMyTimeHistory()}" varStatus="loop">
