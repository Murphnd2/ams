...
<c:choose>
  <c:when test="${sessionScope.sVar.getMyCurrentCheckLists().size()>0}">
    <c:forEach var="list" items="${sessionScope.sVar.getMyCurrentCheckLists()}">
      ...
      <c:choose>
        <c:when test="${list.getToDoCount()==1}">
          <form method="post" action="doCheckListAction" class="mb-1">
            <div class="accordion" id="a${list.getId()}">
              <div class="accordion-item">
                <div class="accordion-header" id="h${list.getId()}">
                  <div class="input-group input-group-sm">
                        <button type="submit" class="btn btn-outline-dark ${butt}" name="btnCheckList" id="btn${list.getId()}-todo" value="C-${list.getId()}">
                  ...
                  <div class="input-group input-group-sm">
                    <c:choose>
                      ...
                      <c:when test="${list.hasDelegate()}">
                        <button type="submit" class="form-control btn btn-outline-primary" name="btnCheckList" id="btn2${list.getId()}" value="V-${list.getId()}">
...
<div class="row">
  <div class="col">
    <c:forEach var="list1" items="${sessionScope.sVar.getMyClosedTodayCheckLists()}">
      <form method="post" action="doCheckListAction" class="mb-1">
        <div class="input-group input-group-sm">
          <button type="submit" class="btn btn-outline-success"  name="btnCheckList" id="btn2${list1.getId()}" value="U-${list1.getId()}">
