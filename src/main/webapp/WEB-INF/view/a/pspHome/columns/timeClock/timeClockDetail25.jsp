...
<form method="post" action="TimeClock25">
  <div class="d-none" id="codeBehindClock">
    ...
    <c:if test="${sessionScope.local.isUserIsIn()==true}">
  ...
  <div class="row">
    <div class="col">
      <div class="btn btn-outline-dark border-0 w-100 pe-none">
        <c:choose><%----%>
          <c:when test="${sessionScope.local.isUserIsIn()==true && sessionScope.local.getMyTimeHistory().size()==0}">
            IN Since Yesterday
          </c:when>
          <c:when test="${sessionScope.local.isUserIsIn()==true}">
            IN Since <fmt:formatDate value="${sessionScope.local.getMyTimeHistory().get(0).getInTime()}" pattern="hh:mm aa"></fmt:formatDate>
          </c:when>
          <c:when test="${sessionScope.local.isUserIsIn()==false && sessionScope.local.getMyTimeHistory().size()==0}">
            OUT Since Yesterday
          </c:when>
          <c:otherwise>
            OUT Since <fmt:formatDate value="${sessionScope.local.getMyTimeHistory().get(0).getOutTime()}" pattern="hh:mm aa"></fmt:formatDate>
          </c:otherwise>
        </c:choose>
      </div>
    </div>
...
<c:forEach var="timeStretch" items="${sessionScope.local.getMyTimeHistory()}" varStatus="loop">
