...
<div class="container-fluid m-0 p-0">
  <div class="row m-0 p-0 overflow-auto" style="max-height:575px">
    <div class="col m-0 p-0">
      ...
      <c:forEach var="toDo" items="${sessionScope.getCurrentOpenToDos()}" varStatus="tdId">
        ...
        <c:if test="${!toDo.getTask().getDescription().equals(\"Default\")}">
          ...
          <div id="massCodeBehind" class="d-none">
            ...
            <c:if test="${toDo.getTask().allowNonOwner() || (toDo.getTask().getOwner()!=null && toDo.getTask().getOwner().getId()==sessionScope.sVar.getCurrentPerson().getId())}">
              <c:set var="onlyOwner" value="N"></c:set>
            </c:if>

            <c:set var="hOwner" value="N"></c:set>
            <c:if test="${toDo.getTask().hasOwner() && toDo.getTask().allowNonOwner() && (toDo.getTask().getOwner().getId()!=sessionScope.sVar.getCurrentPerson().getId())}">
            ...
            <c:if test="${toDo.isComplete() || !toDo.getTask().hasInfo() || toDo.getTask().getInfoLink()==null || (!toDo.getTask().allowNonOwner() && toDo.getTask().getOwner().getId()!=sessionScope.sVar.getCurrentPerson().getId())}">
            ...
            <c:if test="${toDo.getTask().getOwner()!=null && toDo.getTask().getOwner().getId()==sessionScope.sVar.getCurrentPerson().getId()}">
