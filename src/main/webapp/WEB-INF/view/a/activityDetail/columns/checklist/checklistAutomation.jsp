...
<c:choose>
  <c:when test="${sessionScope.sVar.getCurrentActivityToDos().get(0).isComplete()==false && sessionScope.sVar.getCurrentActivityToDos().get(0).hasAutomation()==true}">
    <div class="row m-0 mb-2 p-0">
      <div class="col m-0 p-0">
        <div class="input-group input-group-sm">
          <div class="form-control bg-warning text-dark fw-bold fst-italic border-dark">
              ${sessionScope.sVar.getCurrentActivityToDos().get(0).getAutomationText()}
          </div>
          <c:set var="sName" value="${sessionScope.sVar.getCurrentOpenToDos().get(0).getTask().getServletName()}"></c:set>
          ...
          <a class="btn btn-outline-warning text-dark border-dark" href="${sessionScope.sVar.getCurrentActivityToDos().get(0).getServletName()}">
