      ...
      <div class="col-auto text-muted fst-italic form-label" style="font-size: xx-small">
          ...
          <c:choose>
            <c:when test="${note.getDateCreated()!=null}">
              <fmt:formatDate value="${note.getDateCreated()}" pattern="MM/dd/yy @ hh:mm aa"></fmt:formatDate>
            </c:when>
            <c:otherwise>
              <fmt:formatDate value="${note.getDateGenerated()}" pattern="MM/dd/yy"></fmt:formatDate>
            </c:otherwise>
          </c:choose>
      </div>
    </div>
